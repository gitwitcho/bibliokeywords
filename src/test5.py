...
scopus_df = read_and_merge_csv_files(project = project, 
                                     input_dir = 'raw/scopus',
                                     biblio_type = BiblioType.SCOPUS,
                                     n_rows = 3)

lens_df = read_and_merge_csv_files(project = project, 
                                   input_dir = 'raw/lens',
                                   biblio_type = BiblioType.LENS,
                                   n_rows = 3)

dims_df = read_and_merge_csv_files(project = project, 
                                   input_dir = 'raw/dimensions',
                                   biblio_type = BiblioType.DIMS,
                                   n_rows = 3)

scopus_df = reshape_cols_biblio_df(biblio_df = scopus_df, reshape_base = Reshape.SCOPUS_COMPACT)
lens_df = reshape_cols_biblio_df(biblio_df = lens_df, reshape_base = Reshape.LENS_COMPACT)
dims_df = reshape_cols_biblio_df(biblio_df = dims_df, reshape_base = Reshape.DIMS_COMPACT)
