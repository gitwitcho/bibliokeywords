...
def generate_bert_topics(biblio_df: pd.DataFrame,
    ...
    biblio_stripped_df = biblio_df.dropna(subset = [input_col])    # Remove rows where abstract is NaN
    biblio_stripped_df = biblio_stripped_df[biblio_stripped_df[input_col] != '']   # Remove rows where abstract is an empty string

    # Use all or a subset of the documents
    if sample_size:
        biblio_stripped_df = biblio_stripped_df.sample(n = sample_size)
    
    # Create a list of the titles
    docs = biblio_stripped_df[input_col].to_list()
    ...
    biblio_stripped_df.reset_index(drop = True, inplace = True)
    doc_info_df.reset_index(drop = True, inplace = True)
    merged_df = pd.concat([biblio_stripped_df, doc_info_df], axis = 1)
